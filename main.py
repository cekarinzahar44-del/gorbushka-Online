...
# === БОТ ===
def get_admin_kb():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📊 Заказы")], [KeyboardButton(text="📈 Статистика")], [KeyboardButton(text="🔙 В меню")]], resize_keyboard=True)

def get_user_kb():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📦 Мои заказы")], [KeyboardButton(text="💰 Бонусы")], [KeyboardButton(text="ℹ️ Поддержка")]], resize_keyboard=True)

@dp.message(Command("start"))
async def cmd_start(message: Message):
    conn = get_db_connection()
    conn.execute('INSERT OR IGNORE INTO users (user_id, username) VALUES (?, ?)', (message.from_user.id, message.from_user.username))
    conn.commit()
    conn.close()
    inline_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🛍 ОТКРЫТЬ МАГАЗИН", web_app=WebAppInfo(url=MINI_APP_URL))]])
...
async def my_orders_bot(message: Message):
    await message.answer("📦 <b>История заказов</b>\n\nОткройте магазин и перейдите в Профиль → Мои заказы 👆",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🛍 Открыть магазин", web_app=WebAppInfo(url=MINI_APP_URL))]]), parse_mode="HTML")
...
async def edit_order(callback):
    ...
    builder.button(text="🔙 Назад", callback_data="admin_back")
    builder.adjust(2)
    await callback.message.edit_text(f"📦 Заказ #{oid}\nСтатус: {row['status']}\n\nВыберите:", reply_markup=builder.as_markup())
